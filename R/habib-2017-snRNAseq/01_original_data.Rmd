...
```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "01", "pca.rds")
if (file.exists(rds)) {
  ppk <- readRDS(rds)
} else {
  set.seed(1)
...
```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "01", "pca_umap.rds")
if (file.exists(rds)) {
  umap <- readRDS(rds)
} else {
  set.seed(1)
...
add_df <- data.frame(reducedDim(sce, "UMAP"))
names(add_df) <- paste0("pca_umap", seq(ncol(add_df)))
colData(sce) <- cbind(
  colData(sce), pca_umap1 = add_df$pca_umap1, pca_umap2 = add_df$pca_umap2)
...
label_df2 <- gg_df %>%
  group_by(habib_cluster_name) %>%
  summarize(pca_umap1 = median(pca_umap1), pca_umap2 = median(pca_umap2)) %>%
  left_join(label_df)

dim_red_plot(
  data = gg_df, x = "pca_umap1", y = "pca_umap2", col = "habib_cluster_name", type = "cat") +
  xlab("UMAP 1") + ylab("UMAP 2") + ggtitle("Habib Clusters PCA UMAP")
dim_red_plot(
  data = gg_df, x = "pca_umap1", y = "pca_umap2", col = "cell_id_stem", type = "other") +
  xlab("UMAP 1") + ylab("UMAP 2") + ggtitle("Sample Distribution PCA UMAP")
dim_red_plot(
  data = gg_df, x = "pca_umap1", y = "pca_umap2",
  col = "log10_total_features_by_counts", type = "cont") +
  xlab("UMAP 1") + ylab("UMAP 2") + ggtitle("log10(Total Features) PCA UMAP")
```
...
# Setup for automatic placement of cluster labels.
label_df <- data.frame(
  seurat_clusters = levels(gg_df$seurat_clusters),
  label = levels(gg_df$seurat_clusters))
label_df2 <- gg_df %>%
...
dim_red_plot(
  data = gg_df, x = "pca_umap1", y = "pca_umap2", col = "seurat_clusters", type = "cat") +
  xlab("UMAP 1") + ylab("UMAP 2") + ggtitle("Seurat Clusters PCA UMAP")
```
