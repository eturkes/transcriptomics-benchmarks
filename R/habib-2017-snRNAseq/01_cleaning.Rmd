...
# Clean up `clust` and align rows to those in `colData`.
clust <- clust[ , -c(2:4)]
names(clust) <- c("sample", "clust")
clust <- clust[match(colData$sample, clust$sample), ]
rownames(clust) <- NULL
datatable(clust[1:5, ])
```
...
```{r}
names(tsne) = c("sample", paste0("tsne", 1:2))
datatable(tsne[1:5, ])
```
...
```{r}
colData <- cbind(colData, clust[ , names(clust) != "sample"], tsne[ , names(tsne) != "sample"])
sce <- SingleCellExperiment(assays = list(counts = counts), colData = colData)
sce_orig <- sce
rm(counts, clust, tsne, colData)
sce
```
